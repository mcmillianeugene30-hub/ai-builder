...
By default, deployments are **not ignored**. This avoids accidental non-deploying PRs when Vercel project names differ from expected values.

If you still have a second Vercel project connected to this repo (for example an old `workspace` project),
set `SKIP_VERCEL_DEPLOY=1` in that project’s Vercel environment variables so its builds are skipped.
Leave `SKIP_VERCEL_DEPLOY` unset in your primary `ai-builder` project.

```bash
# only set this in the project you want to disable
export SKIP_VERCEL_DEPLOY=1
